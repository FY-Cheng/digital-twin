    ...
    def process(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.dwaMidposIndex >= 0:
                ...
                self.robotVel, best_traj, all_traj, all_u = self.dwa.plan(
                    [self.robotPos[0],self.robotPos[1],self.robotPos[2],self.robotVel[0],self.robotVel[1]],self.dwaConfig, midpos, self.obsList)
            else:
            ...
            # 运动学控制
            # dx, dy, dw = self.robotVel[0]*self.timestep, 0, self.robotVel[0]*self.timestep
            # T = self.transformation_matrix(self.robotPos[0], self.robotPos[1], self.robotPos[2])
            # p = np.matmul(T, np.array([dx, dy, 1]))
            # self.robotPos = [p[0], p[1], self.robotPos[2] + dw]
